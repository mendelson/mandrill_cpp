...
void MovementProcessor::Update()
{
	...
	//--------------------------------------
	// these next few lines are for
	// creating
	// the frames with motion detection,
	// the real motion deection is done
	// in
	// searchMovement, because the balck
	// and white
	// frame transformation to save can
	// not be used
	...
	// saves second frame as
	// sotredFrame, in order ro be use
	// on the next call of the function,
	// to detect if there are
	// movemente according to the next
	// frame
	_storedFrame	  = *_frame;
	_storedFrameIndex = _currentFrameIndex;

	// adds"~movement processed" image
	// to buffer
	addFrame(thresholdImage);

	// calls function that will
	// identifie
	// if movement occurred
	searchMovement(frame1, frame2);

	// if the buffer passes by MAXSAVING
	// from the
	// time the movement stopped, we
	// save the frames
	// to a file, and clear movement
	// flags
...
{
	_mutex2.lock();
	// Printer::safe_print("ADICIONADO:"
	// +
	// std::to_string(_latestFrameIndex)
	// +
	// "\n");
	// Printer::safe_print("SIZE:" +
	// std::to_string(_cameraBuffer.size())
	// +
	// "\n");
	// Printer::safe_print("SIZEmax:" +
	...
	if(_cameraBuffer.size() >= MAXFRAMES)
	{
		...
		// but frames are in movement
		// range
		// we have to save them in file,
		// before
		// clearing the buffer
		if(_firstFrame != -1)
		{
			Printer::safe_print("EU Q "
								"ADICIONEI\n");
			saveBuffer();
			Printer::safe_print("EU Q "
								"ADICIONEI\n");
	...
	_cameraBuffer.emplace(_latestFrameIndex + 1,
...
{
	// int inicio = (_firstFrame - 10 >=
	// 0) ? (_firstFrame - 10) :0;
	int inicio = 0;
	int i;

	// Printer::safe_print("YOYO AQUI:"
	// + std::to_string(_firstFrame) +
	// "\n");
